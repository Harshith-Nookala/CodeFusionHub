...
(
    select distinct project_id,project_name,dead_line from employees
);
--drop table projects;
...
(
    select distinct client_id,client_org,client_city from employees
);
--drop table clients;
select * from clients;
-- step - 2:
...
--step - 3:
alter table employees drop (project_name,dead_line,client_org,client_city);
-- step -4:
alter table employees add constraint fk_proj foreign key(project_id) references projects(project_id);

alter table employees add constraint fk_client foreign key(client_id) references clients(client_id);